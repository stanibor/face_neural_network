...
def spatial_transforms(min_max_height: Tuple[int, int] = (256, 320), output_size: Tuple[int, int] = (256, 256)):
    transform = A.Compose([A.Rotate(limit=20, p=0.4),
...
def photometric_transforms():
    transform = A.Compose([A.RandomBrightnessContrast(p=0.5),
                           A.OneOf([A.RandomGamma(), A.RandomToneCurve()], p=0.1),
                           A.OneOf([A.RGBShift(), A.Posterize()], p=0.2),
...
def default_transform(min_max_height: Tuple[int, int] = (256, 320),
